...
void GraphOperation::bfs_distance_to_target(DirectedGraph *g, uint32_t u, uint32_t *distance, uint32_t invalid_value) {
    bool* visited = (bool*)malloc(sizeof(bool) * g->num_vertices_);
    memset(visited, 0, sizeof(bool) * g->num_vertices_);
    std::fill(distance, distance + g->num_vertices_, invalid_value);
